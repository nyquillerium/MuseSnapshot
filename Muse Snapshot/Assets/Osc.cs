...
public class Osc
{
    ...
    private void Read()
    { // Read Thread.  Loops waiting for packets.  When a packet is received, it isdispatched to any waiting All Message Handler.  Also, the address is looked up andany matching handler is called.
        try
        {
            while (ReaderRunning)
            {
    ...
    { // Set the method to call back on when a message with the specifiedaddress is received.  The method needs to have the OscMessageHandler signature - i.e.void amh( OscMessage oscM )
    ...
    {// General static helper that returns a string suitable for printing representing the suppliedOscMessage.
